...
static void InitUsart3(const uint32_t baudrate, const uint32_t irq_priority);
...
int main() {
  ...
  InitUsart3(USART3_BAUDRATE, USART3_IRQ_PRIORITY);
...
static void SetUsartBaudRate(USART_TypeDef *const usart,
  ...
  if (usart == USART1) {
    bus_clock = rcc::GetPCLK2Frequency();
  } else {
    bus_clock = rcc::GetPCLK1Frequency();
  }
